...
u8 MPU_Init(void) { 
	u8 res;
    Hard_IIC_Init(MPU_IIC, 0xCC);//初始化IIC总线
	MPU_Write_Byte(MPU_PWR_MGMT1_REG, 0X80);	//复位MPU6050
    delay_ms(200);
    MPU_Write_Byte(MPU_PWR_MGMT1_REG, 0X00);	//唤醒MPU6050 
	MPU_Set_Gyro_Fsr(3);					//陀螺仪传感器,±2000dps
	MPU_Set_Accel_Fsr(0);					//加速度传感器,±2g
    res = MPU_Set_LPF(10); //低通滤波器10Hz
    if (res) {
        printf("Low-pass filter set failed.");
    }
	MPU_Set_Rate(MPU_6050_SAMPLE_FREQUENCY);						//设置采样率100Hz
	...
	if(res == MPU_DEVICE_ID) {//器件ID正确
		MPU_Write_Byte(MPU_PWR_MGMT1_REG,0X01);	//设置CLKSEL,PLL X轴为参考
		MPU_Write_Byte(MPU_PWR_MGMT2_REG,0X00);	//加速度与陀螺仪都工作
		MPU_Set_Rate(MPU_6050_SAMPLE_FREQUENCY);						//设置采样率为100Hz
        return 0;
 	}else{
        return 1;
    }
